...
console.log("Unfortunly, the new dinner table want arrive on time, so i can only invte only two guests to dinner with me");
// using while-loop to remove guests from the Array until only two names remain
while (guestList.length > 2) {
    let RemovedGuest = guestList.pop();
    console.log(`SORRY, ${RemovedGuest} I cannt invite you to dinner`);
}
// sending last 2 Guests invitation
console.log("Invuitation to the last 2 guests");
guestList.forEach(lasttwo => console.log(`lucky${lasttwo}, you are still invated to dinner`));
// removeing two guest from ist 
guestList.pop();
guestList.pop();
